...
def init_form_ranking(dict_form_data: dict, jugador: dict):
    ...
    form['lbl_titulo'] = Label(x=635, y=105, text='DRAGON BALL Z TCG', screen=form.get('screen'), font_path="./modulos/forms/Halimount.otf", font_size=75)
    form['lbl_subtitulo'] = Label(x=635, y=195, text='TOP 10 RANKING', screen=form.get('screen'), font_path="./modulos/forms/Halimount.otf", font_size=55)

    form['boton_volver'] = Button(x=635, y=615, text='VOLVER AL MENU', screen=form.get('screen'), font_path="./modulos/forms/Halimount.otf", color=var.COLOR_NARANJA, font_size=40, on_click=click_return_menu, on_click_param='form_main_menu')
...
def init_ranking(form_data: dict):
    ...
    for indice_fila in range(len(matriz)):
        ...
        form_data['ranking_screen'].append(
                Label(x=435, y=735//2.9 + indice_fila*31,text=f'{indice_fila + 1}', screen=form_data.get('screen'), font_path="./modulos/forms/Halimount.otf", color=var.COLOR_NARANJA, font_size=40)
            )
        
        #nombre
        form_data['ranking_screen'].append(
                Label(x=635, y=735//2.9 + indice_fila*31, text=f'{fila[0]}', screen=form_data.get('screen'), font_path="./modulos/forms/Halimount.otf", color=var.COLOR_NARANJA, font_size=40)
            )
        
        #puntaje
        form_data['ranking_screen'].append(
                Label(x=835, y=735//2.9 + indice_fila*31, text=f'{fila[1]}', screen=form_data.get('screen'), font_path="./modulos/forms/Halimount.otf", color=var.COLOR_NARANJA,font_size=40)
            )
