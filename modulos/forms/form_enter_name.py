...
def init_form_enter_name(dict_form_data: dict, jugador: dict):
    ...
    form['score'] = jugador_mod.get_puntaje_actual(form.get('jugador'))
    ...
    form['title_2'] = Label (
        x=var.DIMENSION_PANTALLA[0] // 2, y=var.DIMENSION_PANTALLA[1] // 2 - 150, text='¡Ganaste!', screen=form.get('screen'), font_path=var.RUTA_FUENTE, font_size=60, color=var.COLOR_NEGRO
    )
    form['subtitle'] = Label (
        x=var.DIMENSION_PANTALLA[0] // 2, y=var.DIMENSION_PANTALLA[1] // 2 - 80, text='Escribe tu nombre: ', screen=form.get('screen'), font_path=var.RUTA_FUENTE, font_size=50, color=var.COLOR_BLANCO
    )
    form['subtitle_score'] = Label (
        x=var.DIMENSION_PANTALLA[0] // 2, y=var.DIMENSION_PANTALLA[1] // 2 - 20, text=F'Score: {form.get("score")}', screen=form.get('screen'), font_path=var.RUTA_FUENTE, font_size=30, color=(0,0,255)
    )

    form['text_box'] = TextBox(
        x=var.DIMENSION_PANTALLA[0] // 2, y=var.DIMENSION_PANTALLA[1] // 2 + 40, 
        text='__________',screen=form.get('screen'), font_path=var.RUTA_FUENTE, font_size=25, color=var.COLOR_NEGRO
    )
...
def draw(form_dict: dict):
    ...
    form_dict['writing_text'] = Label(
        ...
        text=f'{form_dict.get('text_box').writing.upper()}', screen=form_dict.get('screen'), font_path=var.RUTA_FUENTE, font_size=30, color= var.COLOR_BLANCO
    )
